...
public class Unit : MonoBehaviour {
		...
		if (timer > 1f) {
			...
			if (target != null) {
				...
				if ((target.position != currenttarget)||(reachemptytarget==true))
				{
					...
					Debug.Log ("Change path now at " + target.position);
	...
	IEnumerator FollowPath() {
		...
		while (true) {
			...
			if ((transform.position.x - 12f < target.position.x) &&
				(transform.position.x + 12f > target.position.x) &&
				(transform.position.z - 12f < target.position.z) &&
				(transform.position.z + 12f > target.position.z)
				) {
	...
	void AddAllEnemy() {
		...
		string enemy_building_tag = team == 1 ? "building2" : "building1";
